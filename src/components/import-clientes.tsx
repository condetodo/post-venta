...
const TARGET_FIELDS = [
  ...
  { key: 'patente', label: 'Patente', required: false },
  { key: 'kmActual', label: 'Km Actual', required: false },
  { key: 'ultimoService', label: 'Ultimo Service', required: false },
] as const
...
const AUTO_MATCH_PATTERNS: Record<string, string[]> = {
  ...
  telefono: ['tel', 'phone', 'celular', 'movil', 'whatsapp'],
  email: ['email', 'mail', 'correo', 'e-mail'],
  patente: ['patente', 'dominio', 'plate', 'chapa'],
  kmActual: ['km', 'kilometr', 'mileage', 'odometro'],
  ultimoService: ['service', 'servicio', 'ultimo', 'visita', 'fecha'],
}
...
export default function ImportClientes({
  ...
  const parseFile = useCallback(async (selectedFile: File) => {
    ...
    try {
      ...
      setPreviewRows(rows.slice(0, 10).map((r) => r.map((c) => String(c ?? ''))))
  ...
  const handleImport = useCallback(async () => {
    ...
    setImporting(true)
    try {
      const clientes = allRows.map((row) => {
      ...
      const res = await fetch(
        `/api/concesionarios/${concesionarioId}/clientes/import`,
        {
          method: 'POST',
    ...
    return (
      <div className="rounded-xl border border-gray-200 bg-white p-6">
        ...
        <div className="mb-4 space-y-2 text-sm text-gray-700">
          <p>
            <span className="font-medium text-brand-green">{result.imported}</span> clientes importados
          </p>
          {result.skipped > 0 && (
            <p>
